...
- **Persistence:** The server reads and updates the **same versioned state** as the extension ([architecture.md](architecture.md) — `timeKeeper-state.v1.json` and schema rules). Implementation must avoid corrupting snapshots (reuse the extension’s invariants: no overlapping active intervals, atomic replace where applicable). If two writers can run concurrently (extension UI + MCP), the implementation documents **coordination** (e.g. single-writer bridge, retry on conflict, or advisory locking).
